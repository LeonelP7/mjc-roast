...
Single core — multiple tasks making progress through context switching, but never truly simultaneously..

When an application is parallel but not concurrent?::
This is actually theoretically impossible — parallelism implies multiple tasks running simultaneously, which by definition is concurrency. This scenario doesn't really exist.

When an application is neither concurrent not parallel?::
Single threaded, sequential execution — one task at a time, no switching, no simultaneity.

When an application is both concurrent and parallel?::
Multi-core system with multiple threads — tasks run truly simultaneously across cores, with context switching handling any excess tasks beyond the core count.
