...
* Why arrayDeque works as a stack?
...
* Which Map implement is optimized for thread safety? X
* How does ConcurrentHashMap implements scalability, still maintaining a good performance? X
...
* What is the meaning of term fail-fast and fail-safe in context of collection iteration?
